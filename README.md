...
# 2. Navigate to Data_augmentation/01.SMOTE.ipynb
# 3. Execute all cells to:
#    - Load hyperspectral data
#    - Apply SMOTE augmentation
#    - Generate synthetic samples (2K per class)
```

### CNN-CVAE Training

```bash
# 1. Open the CVAE training notebook
jupyter notebook Data_augmentation/04.SpectraGeni/train_CNN_CVAE.ipynb

# 2. Configure in constants.py:
#    - NUM_EPOCHS = 5000
#    - BATCH_SIZE = 32
#    - LATENT_DIM = 64
#    - NUM_CLASSES = 6

# 3. Run training pipeline (GPU recommended)
#    - Expected training time: 2-4 hours (GPU), 8-12 hours (CPU)

# 4. Generates: 5000 samples per crop class
```
...
#    - Train 15+ base classifiers
#    - Create voting & stacking ensembles
#    - Output classification metrics (Accuracy, F1, Precision, Recall)
```
...
**Linear Models**: Logistic Regression, Ridge Classifier, SGD Classifier, Passive-Aggressive Classifier

**Tree-Based Models**: Decision Tree, Extra Tree, Random Forest, Extra Trees Classifier

**Ensemble Methods**: Gradient Boosting Classifier, AdaBoost Classifier, Bagging Classifier, Voting Classifier, Stacking Classifier

**Distance-Based**: K-Nearest Neighbors (KNN)

**Probabilistic**: Gaussian Process Classifier, Gaussian Naive Bayes, Linear Discriminant Analysis (LDA), Quadratic Discriminant Analysis (QDA)

**SVM Variants**: Support Vector Classifier (SVC)

## Quality Metrics

The project evaluates synthetic data quality using:

| Metric                          | Description                                                                |
| ------------------------------- | -------------------------------------------------------------------------- |
| **KL Divergence**               | Kullback-Leibler divergence between real and synthetic distributions       |
| **Hellinger Distance**          | Symmetric distance measure between probability distributions               |
| **Spectral Angle Mapper (SAM)** | Angular distance in spectral space (hyperspectral-specific)                |
| **Statistical Tests**           | Kolmogorov-Smirnov test, Anderson-Darling test for distribution similarity |

## Results & Performance

### Expected Improvements

- **Baseline (Real Data Only)**: ~75-80% accuracy
- **With SMOTE**: ~82-85% accuracy
- **With BSMOTE**: ~83-86% accuracy
- **With CTGAN**: ~85-88% accuracy
- **With CNN-CVAE**: ~88-92% accuracy (best performance)
- **With Ensemble + CNN-CVAE**: ~90-94% accuracy

### Key Findings

1. **CNN-CVAE** outperforms traditional augmentation methods due to learned latent representations
2. **Ensemble methods** (Voting, Stacking) consistently improve performance by 2-4%
3. **Distribution similarity**: CNN-CVAE generates data with <0.05 KL divergence (near real distribution)
4. **Data efficiency**: 80% real + 20% CNN-CVAE synthetic maintains 99% of performance vs. 100% real data

## Citation

If you use SpectraGeni in your research or projects, please cite:

```bibtex
@software{spectrageni2026,
